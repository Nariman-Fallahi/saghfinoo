    ...
    <>
      ...
      {!hasData && (
        <ProfileEmptyState
          ...
          icon="/icons/common/empty-ads.svg"
          titleBtn="ثبت آگهی"
          linkBtn="/adPosting"
        />
      )}
        ...
        <>
          ...
          <div className="flex flex-wrap justify-between w-full mt-5 gap-y-5">
            {data.data.map((item) => (
              <div
                key={item.id}
                className="w-[48%] lg:w-[31%] rounded-xl border border-gray-100 shadow-xs flex flex-col relative overflow-hidden"
              >
                <Image
                  width={400}
                  height={300}
                  className="w-full h-32 md:h-44 object-cover"
                  src={item.imageFullPath || "/images/common/bg-search.webp"}
                  alt="Ad Image"
                />
