...
import RealtorsSkeleton from "./ui/skeletons/RealtorsSkeleton";

type RealatorsCardProps = {
  data: allRealtorDataType[] | undefined;
  isLoading: boolean;
};

export default function RealatorsCard({
  data,
  isLoading,
}: RealatorsCardProps) {
  const router = useRouter();

  if (isLoading || !data) return <RealtorsSkeleton count={6} />;
        ...
        {data?.map((item) => (
          ...
          >
            ...
            <Button
              ...
              variant="bordered"
              color="danger"
              onPress={() => router.push(`/realator-profile/${item.id}`)}
            >
              نمایش پروفایل
            </Button>
          </div>
